...
module.exports = function() {
  return co(function *() {
    try {
      ...
      // 复制隐藏文件中唯一有用的 babelrc
      yield q.nfcall(exec, 'cp -r ' + path.join(__dirname, '../../src/.babelrc') + ' ' + path.join(dir, '/src'));
          ...
          appenders: [
            {
              category: 'normal',
              type: 'dateFile',
              filename: path.join(dir, '/logs/log'),
      ...
      console.log('数据库与配置文件成成功，请移步 ' + path.join(dir, '/src') + ' 目录下执行 ./preinstall 完成 npm 安装');
